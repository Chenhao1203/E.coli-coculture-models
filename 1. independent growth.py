...
class Strain_h1(Multi_c):#Strain1
    def __init__(self):
        super().__init__()
        self.Kh1u=0.09
        self.Ksh2x=0.01
...
class Strain_h2(Multi_c):#Strain2
    def __init__(self):
        super().__init__()
        self.Kh2u=1.0
        self.Ksh1x=0.01
...
class Bio_r():#bioreactor
    def __init__(self):
        ...
        self.dt=0.01#The time interval
        self.t0=0
        self.cg0=20#initial Glc Conc.
        self.ch1_0=0#initial H1 Conc.
        self.ch2_0=0#initial H2 Conc.
...
def topdu(name):#Different um2；independent growth strains have no Khu or Yxh, and Yhg was set 1 to avoid a zero divisor error.
    pdch1=pd.DataFrame()
    pdch2=pd.DataFrame()
    pdx1=pd.DataFrame()
    pdx2=pd.DataFrame()
...
topdu('independent.pickle')

##############################↓↓↓↓↓↓↓↓↓↓Export the file to CSV format↓↓↓↓↓↓↓↓###############################


try:
    os.makedirs('./independent/')
except:
    pass
with open('independent.pickle','rb') as usr_file:    
    pd1=pickle.load(usr_file)
pdx1,pdx2,pdcg,pdch1,pdch2,ulist,pdx1ratio=pd1


pdx1ratio.to_csv('./independent/x1ratio.csv')
pdX1X2=3*pdx1+3*pdx2
pdX1X2.to_csv('./independent/ODsum.csv')
pdch1.to_csv('./independent/cH1.csv')
pdch2.to_csv('./independent/cH2.csv')
